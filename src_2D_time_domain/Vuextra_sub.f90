...
!     ELEMENTI SEPARATI
  ...
  !Variabili locali
  INTEGER(kind=4) :: ind_gauss, Ngauss, AllocateStatus, iplog, iqlog, ki, kj, Ngaussi, ii, jj
   
  REAL(kind=8) :: delta_x, alfa, beta, p2, A1, B1, alfa_j1, beta_j1, p2a, s, ds, xtrasl, xinttrasl, r2_1, serv, sm1, cs, cp
  ...
  REAL(kind=8):: estremo_m, estremo_m_tilde, &
				 ...
				 Vuextra_sub_P, Vuextra_sub_S, deltaquartiS_bis, deltaquartiP_bis, deltaquartiS_bbis, deltaquartiP_bbis, app, radice_incriminata
         ...
         if(xx(ii+1)-xx(ii).gt.1.d-14)then		 
    	     ...
    	     DO ki=1,Ngauss 
				 ...
				 IF ((B1-A1).gt.10.d-14) THEN
					 ...
					 !write(*,*) 'entro in integraione interna'
                     ...
                     DO kj=1,Ngauss
						 ...
						 !p2a=r(indice_i)*r(indice_j)/(r2_1**2)!!!!!!!!!!!!!!!! (giusto)
						 !p2a=coeff_delta_kronecker/r2_1 (giusto)!!!!!!!!!!!!!!!!
						 !p2a=dlog(cs*delta_x+sqrt((cs*delta_x)**2-r2_1))-dlog(sqrt(r2_1))!!!!!!!!! (non giusto)
						 !p2a=(cs*delta_x)**2-r2_1
	                     ...
	                     IF (delta_kronecker(indice_i,indice_j).eq.1.d0) THEN
	                         ...
	                         !p2a= p2a+wint(kj)*ds*fiU(l_m,serv,estremo_m,grado_q)*coeff_delta_kronecker*(1/cs**2)*(dlog(cs*delta_x+sqrt((cs*delta_x)**2-r2_1))-dlog(sqrt(r2_1)))
	                     ENDIF
						 
						 !if(ii.eq.3.and.ki.eq.32) write(*,*) 'kj,valori interni', kj,p2a!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		                
                     END DO

        	     ENDIF
                 
				 !radice_incriminata=-(CBCFCECC*xtrasl)**2+2*CBCFCECC*(-CFCACCCD)*xtrasl-(CFCACCCD)**2+cs**2*delta_x**2
				 !if(ii.eq.3.and.ki.eq.32) write(*,*) 'ki,A1,B1', ki,A1,B1 !!!!!!!!!!!!!!!!!!!!!!
			 ...
			 ! write(*,*) 'indice splitting points', ii!!!!!!!!!!!!!!!!!!!!!!
			 ! write(*,*) 'ampiezza sotto intervalli', alfa*2 !!!!!!!!!!!!!!!!
			 ! write(*,*) 'contributo integrale', p2*alfa!!!!!!!!!!!!!!!!!!!!!!
			 ! write(*,*) 'flag_extra', flag_extra!!!!!!!!!!!!!!!!!!!!!!
			 ! pause
		 else
